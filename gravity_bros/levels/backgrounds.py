...
def _draw_cloud(surf, x, y, w, h, color=(255, 255, 255), alpha=120):
    ...
    c = (*color, alpha)
    cx, cy = w // 2 + 10, h // 2 + 10
    pygame.draw.ellipse(cs, c, (cx - w // 3, cy - h // 3, w * 2 // 3, h * 2 // 3))
...
def _build_banaue(w, h):
    ...
    _gradient_fill(surf, (135, 200, 255), (200, 240, 255))
    # Distant misty mountains
    _draw_mountain_range(surf, h * 0.45, [
    ...
    for i, tc in enumerate(terrace_colors):
        ty = int(h * 0.50 + i * 28)
        pygame.draw.rect(surf, tc, (0, ty, w, 28))
        # Terrace edge highlight
        pygame.draw.line(surf, (tc[0] + 30, tc[1] + 30, tc[2] + 10), (0, ty), (w, ty), 2)
        # Water reflection in paddy
        water_c = (100, 200, 220, 80)
        ws = pygame.Surface((w, 8), pygame.SRCALPHA)
        ws.fill(water_c)
        surf.blit(ws, (0, ty + 10))
    # Scattered huts
    for hx in [w * 0.2, w * 0.65]:
        pygame.draw.rect(surf, (120, 70, 30), (hx, h * 0.44, 20, 15))
        pygame.draw.polygon(surf, (80, 50, 20), [(hx - 3, h * 0.44), (hx + 10, h * 0.44 - 12), (hx + 23, h * 0.44)])
    return surf
...
def _build_chocolate_hills(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (150, 220, 255), (220, 240, 255))
    # Layered hills
    random.seed(42)
    for layer in range(3):
        ...
        for i in range(8):
            ...
            pygame.draw.circle(surf, c, (int(cx), int(base_y)), cr)
            # Highlight
            pygame.draw.arc(surf, (shade + 20, shade - 20, shade - 60),
                            (int(cx) - cr, int(base_y) - cr, cr * 2, cr * 2),
                            0.3, 2.8, 2)
    # Green grass base
    pygame.draw.rect(surf, (80, 140, 50), (0, int(h * 0.78), w, int(h * 0.22)))
    _gradient_fill(pygame.Surface((1, 1)), (80, 140, 50), (60, 100, 30))  # just to avoid error
    # Scattered palm trees
    for tx in range(50, w, 180):
        _draw_palm_tree(surf, tx + random.randint(-15, 15), int(h * 0.78), 40)
    random.seed()
    return surf
...
def _build_boracay(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (255, 150, 80), (100, 200, 255))
    # Sun
    pygame.draw.circle(surf, (255, 220, 100), (int(w * 0.75), int(h * 0.2)), 40)
    pygame.draw.circle(surf, (255, 240, 180), (int(w * 0.75), int(h * 0.2)), 50, 3)
    # Ocean layers
    ocean_colors = [(0, 150, 200), (0, 170, 220), (20, 190, 230), (60, 210, 240)]
    for i, oc in enumerate(ocean_colors):
        oy = int(h * 0.50 + i * 25)
        pygame.draw.rect(surf, oc, (0, oy, w, 25))
    # White sand beach
    pygame.draw.rect(surf, (245, 235, 200), (0, int(h * 0.75), w, int(h * 0.25)))
    pygame.draw.rect(surf, (235, 225, 190), (0, int(h * 0.80), w, int(h * 0.20)))
    # Foam line
    for fx in range(0, w, 12):
        fy = int(h * 0.75) + random.randint(-2, 2)
        pygame.draw.circle(surf, (255, 255, 255), (fx, fy), random.randint(3, 6))
    # Palm trees on beach
    for tx in [w * 0.1, w * 0.35, w * 0.7, w * 0.9]:
        _draw_palm_tree(surf, int(tx), int(h * 0.75), 55, (20, 120, 20))
    # Sailboat
    bx, by = int(w * 0.5), int(h * 0.55)
    pygame.draw.polygon(surf, (200, 180, 150), [(bx - 15, by), (bx + 15, by), (bx + 5, by + 10), (bx - 5, by + 10)])
    pygame.draw.line(surf, (120, 80, 40), (bx, by), (bx, by - 25), 2)
    pygame.draw.polygon(surf, (255, 255, 255), [(bx, by - 25), (bx + 15, by - 10), (bx, by - 5)])
    return surf
...
def _build_palawan_cave(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (10, 15, 30), (20, 40, 60))
...
def _build_mayon(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (60, 10, 10), (180, 60, 20))
    # Volcanic ash clouds
    for _ in range(6):
        _draw_cloud(surf, random.randint(0, w), random.randint(10, int(h * 0.3)),
                    random.randint(80, 140), random.randint(30, 50), (80, 40, 30), 100)
    # The volcano (center)
    vx = w // 2
    pygame.draw.polygon(surf, (60, 40, 30), [(vx - 200, h * 0.85), (vx, h * 0.15), (vx + 200, h * 0.85)])
    pygame.draw.polygon(surf, (80, 50, 35), [(vx - 180, h * 0.85), (vx, h * 0.18), (vx + 100, h * 0.85)])
    # Crater glow
    pygame.draw.ellipse(surf, (255, 100, 0), (vx - 15, h * 0.14, 30, 12))
    pygame.draw.ellipse(surf, (255, 200, 50), (vx - 8, h * 0.145, 16, 6))
    # Lava streams
    random.seed(45)
    for i in range(4):
        ...
        for j in range(len(points) - 1):
            pygame.draw.line(surf, (255, random.randint(80, 180), 0), points[j], points[j + 1], 3)
    # Ground lava glow
    lava = pygame.Surface((w, 40), pygame.SRCALPHA)
    _gradient_fill(lava, (255, 80, 0), (200, 30, 0))
    lava.set_alpha(150)
    surf.blit(lava, (0, h - 40))
    random.seed()
    return surf
...
def _build_pulag(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (180, 120, 200), (220, 240, 255))
    # Distant mountain silhouettes (dark blue)
    _draw_mountain_range(surf, h * 0.55, [
    ...
    _draw_mountain_range(surf, h * 0.65, [
    ...
    for i in range(12):
        ...
        _draw_cloud(surf, cx, cy, random.randint(100, 200), random.randint(25, 45), (255, 255, 255), 160)
    # Pine trees in foreground
    for tx in range(30, w, 80):
        _draw_pine_tree(surf, tx + random.randint(-10, 10), int(h * 0.82), random.randint(30, 50))
    # Grass base
    pygame.draw.rect(surf, (50, 100, 40), (0, int(h * 0.82), w, int(h * 0.18)))
    return surf
...
def _build_tubbataha(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (0, 30, 100), (0, 80, 140))
    ...
    for _ in range(8):
        ...
        pygame.draw.ellipse(surf, fc, (fx, fy, 12, 6))
        pygame.draw.polygon(surf, fc, [(fx + 12, fy + 3), (fx + 18, fy - 1), (fx + 18, fy + 7)])
    random.seed()
    return surf
...
def _build_kawasan(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (150, 220, 200), (80, 180, 140))
    # Cliff walls on sides
    pygame.draw.rect(surf, (60, 90, 50), (0, 0, 80, h))
    pygame.draw.rect(surf, (60, 90, 50), (w - 80, 0, 80, h))
    pygame.draw.rect(surf, (50, 80, 40), (0, 0, 70, h))
    pygame.draw.rect(surf, (50, 80, 40), (w - 70, 0, 70, h))
    # Waterfall (center)
    wf_x = w // 2 - 20
    wf_w = 40
    ...
    for side in [0, w]:
        for i in range(8):
            ...
            pygame.draw.ellipse(surf, leaf_c, (tx, ty, random.randint(20, 45), random.randint(15, 30)))
    # Mist
    for _ in range(6):
...
def _build_vigan(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (255, 180, 80), (255, 220, 130))
    # Warm sunset clouds
    for _ in range(5):
        _draw_cloud(surf, random.randint(0, w), random.randint(20, int(h * 0.25)),
    ...
    while bx < w:
        ...
        _draw_building(surf, bx, int(h * 0.78), bw, bh, wall, roof, True)
        bx += bw + random.randint(3, 10)
    # Cobblestone road
    road_y = int(h * 0.78)
    pygame.draw.rect(surf, (120, 100, 70), (0, road_y, w, h - road_y))
    for sy in range(road_y + 4, h, 12):
        for sx in range(0, w, 14):
            sc = random.randint(90, 130)
            pygame.draw.rect(surf, (sc, sc - 10, sc - 25), (sx + (sy % 2) * 7, sy, 10, 8), border_radius=2)
    # Calesa (horse carriage) silhouette
    cx = int(w * 0.6)
    cy = int(h * 0.78) - 5
    pygame.draw.circle(surf, (60, 40, 20), (cx, cy), 12, 2)
    pygame.draw.circle(surf, (60, 40, 20), (cx + 30, cy), 12, 2)
    pygame.draw.rect(surf, (100, 60, 30), (cx - 5, cy - 25, 40, 20))
    random.seed()
    return surf
...
def _build_manila(w, h):
    ...
    surf = pygame.Surface((w, h))
    _gradient_fill(surf, (5, 5, 30), (20, 15, 60))
    # Stars
    random.seed(50)
    for _ in range(60):
        sx = random.randint(0, w)
    ...
    # Moon
    pygame.draw.circle(surf, (220, 220, 240), (int(w * 0.8), int(h * 0.12)), 25)
    pygame.draw.circle(surf, (200, 200, 220), (int(w * 0.8) + 5, int(h * 0.12) - 3), 22)
    # Skyscrapers
    ...
    while bx < w:
        ...
        pygame.draw.rect(surf, bc, (bx, building_base - bh, bw, bh))
        # Lit windows
        ...
        if bh > 150:
            pygame.draw.line(surf, (100, 100, 100), (bx + bw // 2, building_base - bh), (bx + bw // 2, building_base - bh - 20), 2)
            pygame.draw.circle(surf, (255, 0, 0), (bx + bw // 2, building_base - bh - 20), 2)
